...
import { ICreateUE } from "@/types/programTypes";

interface DialogUpdateUEProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSave: (data: ICreateUE) => Promise<boolean>;
  moduleCode: string;
  ueData: ICreateUE | null;
}

export function DialogUpdateUE({
  open,
  onOpenChange,
  onSave,
  moduleCode,
  ...
  const [formData, setFormData] = useState<Partial<ICreateUE>>({});
  ...
  const handleSave = async () => {
    ...
    try {
      const result = await onSave({ ...(formData as ICreateUE), module_code: moduleCode });
      if (!result) return;
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSave} disabled={isSubmitting}>
            {isSubmitting ? "Mise à jour..." : "Mettre à jour l'UE"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
